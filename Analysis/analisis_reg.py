...
def LoadInfo(paramName, metric_list, project_name='DAFS_v2', sweep='pxcw9b3s', plot_fig=''):
    ...
    for i, run in enumerate(runs):
        ...
        #                 file.download('Analysis/file_reg/', replace=True)
...
def G_table(wandb_data, item='best_test_auc'):
    ...
    for f in fill_list:
        ...
        for m in method_list:
            ...
            best_auc_index = wandb_data_fill_model['best_test_mse'].argmax()
...
if __name__ == "__main__":
    ...
    sweep_list = {
        'test_all_feature_v3': 'u9rsuprh', #'jyr2r525'
    }
    ...
    for test_str in test_str_list:
        ...
        for sweep in sweep_list.keys():
            ...
            tab_dict = G_table(wandb_data, item='best_test_mse')
            df = pd.DataFrame.from_dict(tab_dict)
            print(df.T)
            df.to_csv('best_test_mse.csv')

            tab_dict = G_table(wandb_data, item='best_test_r')
            df = pd.DataFrame.from_dict(tab_dict)
            print(df.T)
            df.to_csv('best_test_r.csv')
